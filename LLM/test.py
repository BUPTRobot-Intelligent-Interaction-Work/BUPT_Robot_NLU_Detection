...
messages = [{'role': 'system', 'content': '你好，我是展品导览机器人小达。有什么我可以帮助你的呢？如果你对任何展品感兴趣或者需要导览，都可以告诉我哦！'},
            ...
            {'role': Role.ASSISTANT, 'content': '你好！这里有超过70个展品，包括名画《蒙娜丽莎》、雕塑《大卫》和埃及法老图坦卡蒙的黄金面具等。您可以根据自己的兴趣和喜好选择参观。如果您对任何展品感兴趣或者需要导览，都可以告诉我哦！'},
            {'role': Role.USER, 'content': '再见'},
            {'role': Role.ASSISTANT, 'content': '再见，祝您在博物馆里玩得开心！'}]


def call_with_message(question: str, messages: list = messages) -> str:
...
if __name__ == '__main__':
    answer = call_with_message('你好，你是谁？')
    print(answer)
    # answer = call_with_message('给我简单介绍一下这幅叫蒙娜丽莎的画作吧。')
    # print(answer)
    answer = call_with_message('最后的晚餐是什么画作？')
    print(answer)
    # answer = call_with_message('来聊聊最近的好看电影吧。')
    # print(answer)
    answer = call_with_message('再见')
    print(answer)
